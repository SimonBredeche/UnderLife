...
public class Janga extends LivingEntity{
	int phase = 0;
	private boolean onGround = false;
	private Animation[] janganim = new Animation[3];
	public Janga(Animation mobAnim[], String name, Rectangle rect, int life, boolean pacific, float speed, int damage) {
    ...
    protected float getFuturY(int delta) {
        ...
        if(!onGround) {
        	futurY = (this.getY()) + this.getSpeed() * delta;
        }
        return futurY;
    }
	...
	public void aggroAI(int delta, double distance) {
		if(distance > detectionrange) {
			...
			if(this.tick > 25) {
				try {
					Projectile temp;
					temp = (Projectile) ProjectileManager.shampi.clone();
					temp.setAlly(false);
